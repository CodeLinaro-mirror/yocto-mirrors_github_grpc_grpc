...
Http2Status ValidateFrameHeader(const uint32_t max_frame_size_setting,
  ...
  // For servers, if the stream id is not known, it means the Headers have not
  // been received for this stream yet. This means that the stream is still in
  // idle state. A stream in idle state cannot receive any frames other than
  // HEADERS or PRIORITY.
  if (GPR_UNLIKELY(current_frame_header.stream_id > last_stream_id)) {
    ...
    if (GPR_UNLIKELY(is_client || (!is_client && !is_metadata_frame))) {
      return Http2Status::Http2ConnectionError(
          Http2ErrorCode::kProtocolError,
          std::string(is_client ? RFC9113::kUnknownStreamId
                                : RFC9113::kIdleStreamError));
    }
  }
