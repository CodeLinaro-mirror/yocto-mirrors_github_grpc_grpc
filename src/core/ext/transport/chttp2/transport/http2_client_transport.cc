...
void Http2ClientTransport::StartCall(CallHandler call_handler) {
  ...
  call_handler.SpawnGuarded("OutboundLoop", [this, call_handler]() mutable {
    std::optional<RefCountedPtr<Stream>> stream =
        MakeStream(std::move(call_handler));

    return UntilTransportClosed(If(
        stream.has_value(),
        [this, stream]() mutable {
          return Map(CallOutboundLoop(std::move(stream.value())),
