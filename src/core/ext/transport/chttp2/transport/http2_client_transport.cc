...
Http2Status Http2ClientTransport::ProcessHttp2GoawayFrame(
  ...
  {
    MutexLock lock(&transport_mutex_);
    // If there are no active streams and the next stream id is greater than the
    // max allowed stream id, no new streams can be created and hence the
...
Http2ClientTransport::DequeueStreamFrames(RefCountedPtr<Stream> stream) {
  ...
  if (result.InitialMetadataDequeued()) {
    GRPC_HTTP2_CLIENT_DLOG
        << "Http2ClientTransport MultiplexerLoop InitialMetadataDequeued "
           "stream_id = "
        << stream->GetStreamId();
...
void Http2ClientTransport::AddToStreamList(RefCountedPtr<Stream> stream) {
  ...
  GRPC_DCHECK_GT(stream->GetStreamId(), 0) << "stream id is invalid";
  GRPC_HTTP2_CLIENT_DLOG
