  ...
  switch (promise.promise_case()) {
    case grpc::channelz::v2::Promise::kSeqPromise: {
      ...
      for (const auto& step : seq.steps()) {
        ...
        if (step.has_polling_promise()) {
          absl::StrAppend(&out, "🟢 ", FormatFactory(step.factory()), "\n");
          absl::StrAppend(&out, std::string(indent + 4, ' '));
          PromiseFormatterImpl(step.polling_promise(), out, indent + 4);
          absl::StrAppend(&out, ",\n");
        } else {
          absl::StrAppend(&out, FormatFactory(step.factory()), ",\n");
        }
    ...
    case grpc::channelz::v2::Promise::kJoinPromise: {
      ...
      for (const auto& branch : join.branches()) {
        ...
        if (branch.has_polling_promise()) {
          absl::StrAppend(&out, "🟢 ", FormatFactory(branch.factory()), "\n");
          absl::StrAppend(&out, std::string(indent + 4, ' '));
          PromiseFormatterImpl(branch.polling_promise(), out, indent + 4);
          absl::StrAppend(&out, ",\n");
    ...
    case grpc::channelz::v2::Promise::kLoopPromise: {
      ...
      absl::StrAppend(&out, "Loop(\n", std::string(indent + 2, ' '),
                      FormatFactory(loop.loop_factory()), ",\n",
                      std::string(indent + 2, ' '));
