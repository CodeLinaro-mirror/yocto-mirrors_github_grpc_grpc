...
TEST_F(TransportTest, DeferTcpTracerInitialization) {
  ...
  // Set the context AFTER the call has been started (and CallOutboundLoop
  // created).
  call_handler->arena()->SetContext<CallTracer>(call_tracer);
  ...
  call_handler->SpawnInfallible("test-io", [handler = *call_handler]() mutable {
    return Seq(
        [handler]() mutable {
          return handler.PushServerInitialMetadata(TestInitialMetadata());
        },
