...
TEST_F(TransportTest, DeferTcpTracerInitialization) {
  ...
  builder.AddOnClientInitialMetadata(
      [release_metadata, waker_shared](ClientMetadata&) {
        return [release_metadata, waker_shared]() -> Poll<absl::Status> {
  ...
  // Set the context AFTER the call has started but BEFORE the metadata
  // resolves.
  arena->SetContext<CallTracer>(call_tracer);
