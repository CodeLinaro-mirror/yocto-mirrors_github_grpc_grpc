...
// Also verifies that the server sends trailing metadata with the failed status
// and message to the client.
...
CORE_END2END_TEST(CoreEnd2endTests, FilterCausesCloseWithTrailers) {
  CoreConfiguration::RegisterEphemeralBuilder(
      [](CoreConfiguration::Builder* builder) {
  ...
  // recv_message callback with an absl::OkStatus() and hence the RecvMessage op
  // does not fail.
  bool supports_request_proxying =
      test_config()->feature_mask & FEATURE_MASK_SUPPORTS_REQUEST_PROXYING;
  Expect(102, supports_request_proxying);
  Expect(3, true);
  Expect(1, true);
  Step();
