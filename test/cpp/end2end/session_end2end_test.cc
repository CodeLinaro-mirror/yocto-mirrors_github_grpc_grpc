...
class SessionEnd2endTest : public ::testing::Test {
 protected:
  ...
  void SetupVirtualChannel(
    ...
    EXPECT_TRUE(channel_->WaitForConnected(gpr_time_add(
    ...
    auto* session_reactor = new TestSessionReactor(
        [this](grpc::internal::Call call) {
          session_channel_ =
              grpc::experimental::CreateVirtualChannel(call);
...
TEST_F(SessionEnd2endTest, UnaryRpcOverVirtualChannel) {
  ...
  vdone.WaitForNotificationWithTimeout(absl::Seconds(20));

  server_reactor->Close();
  done_.WaitForNotificationWithTimeout(absl::Seconds(20));
}
...
TEST_F(SessionEnd2endTest, StreamingRpcOverVirtualChannel) {
  ...
  absl::Notification vdone;
  ClientContext vcontext;
  vcontext.set_deadline(gpr_time_add(gpr_now(GPR_CLOCK_MONOTONIC),
...
TEST_F(SessionEnd2endTest, SessionGracefulShutdown) {
  ...
  vready.WaitForNotificationWithTimeout(absl::Seconds(20));
  ...
  vdone_after.WaitForNotificationWithTimeout(absl::Seconds(20));
}
...
TEST_F(SessionEnd2endTest, SetupTransportFails) {
  ...
  EXPECT_TRUE(vdone.WaitForNotificationWithTimeout(absl::Seconds(20)));
  EXPECT_TRUE(done_.WaitForNotificationWithTimeout(absl::Seconds(20)));
